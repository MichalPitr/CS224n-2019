...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 2
        enc_hiddens, (h_final, c_final) = self.encoder(nn.utils.rnn.pack_padded_sequence(X, source_lengths))
        enc_hiddens, len_list = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        # 3
        # init decoder hidden
        h_final_concat = torch.cat((h_final[0, :, :], h_final[1, :, :]), 1)
        init_decoder_hidden = self.h_projection(h_final_concat)
        # init decoder cell
        c_final_concat = torch.cat((c_final[0, :, :], c_final[1, :, :]), 1)
        init_decoder_cell = self.c_projection(c_final_concat)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #2 make Y tensor using embeddings
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            ...
            (state, cell), o_t, e_t = \
                self.step(Ybar_t=Ybar_t, dec_state=dec_state, enc_hiddens=enc_hiddens,
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #3 dec_hidden = b*h*1 X enc_hiddens_proj = b*srclen*h = (b*srclen*1)
        ...
        #1 must apply softmax for each batch so dim = 1
        aplha_t = nn.functional.softmax(e_t, dim=1)

        #2 alpha = (b, src_len) enc_hiddens = (b, src_len, 2h) a_t = alpha * enchidden = (b, 2h)
        alpha_t = torch.unsqueeze(aplha_t, dim=1)
