...
def pad_sents_char(sents, char_pad_token):
    ...
    pad_word = [char_pad_token for i in range(max_word_length)]
    sents_padded = []
    for sent in sents:
        #iterate over sents
        #pad all words
        padded_sent = []
        for word in sent:
            add_pad_chars = max_word_length - len(word)
            if add_pad_chars > 0:
                word = word + [char_pad_token for i in range(add_pad_chars)]
            elif add_pad_chars < 0:
                word = word[0:max_word_length]
            padded_sent.append(word)
        if max_sent_len > len(padded_sent):
            add_pad_words = max_sent_len - len(padded_sent)
            padded_sent = padded_sent + [pad_word for i in range(add_pad_words)]
...
def pad_sents(sents, pad_token):
    ...
    sents_padded = []
    for sent in sents:
        add_tokens = max_len - len(sent)
        sent = sent + [pad_token for i in range(add_tokens)]
