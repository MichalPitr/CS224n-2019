...
class CharDecoder(nn.Module):
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        input = char_sequence[:-1, :]  # cut <end> token from input
        output = char_sequence[1:, :]  # cut <start> token from output

        target = output.reshape(-1)  # unroll target into a vector (batch*(length-1))

        scores, dec_hidden = self.forward(input, dec_hidden)  # (length, batch, self.vocab_size)
        scores_reshaped = scores.reshape(-1, scores.shape[2])  # reshape into ((length-1)*batch, self.vocab_size)

        loss_fn = nn.CrossEntropyLoss(ignore_index=self.target_vocab.char2id['<pad>'], reduction='sum')
        return loss_fn(scores_reshaped, target)
        ### END YOUR CODE
